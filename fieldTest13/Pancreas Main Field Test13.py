                ...
                if logPath == True:
                    if (perf_counter()-CurrentTime) > powerReadingDelay:
                        ...
                        try:
                            ...
                            pass
                    try:
                        [lattitude, longitude, x, y, angle, sats, time, quality] = methods.get_filtered_state(aspectRatio, receiver, remoteTransmitter)
                        if sats != 0:
                            lat.append(lattitude[0])
    ...
    #--------------------
            ...
            if userFilename != 'CANCEL':
                try:
                    ...
                    receiver.send_data_async(remoteTransmitter, "Path following terminated")
                except Exception as e:
                    exception_type, exception_object, exception_traceback = sys.exc_info()
                    filename = exception_traceback.tb_frame.f_code.co_filename
                    line_number = traceback.extract_tb(exception_traceback)
                    receiver.send_data_async(remoteTransmitter, str(e) +", "+ str(exception_type)+", "+str(filename)+", "+str(line_number))
                    print(str(e) +", "+ str(exception_type)+", "+str(filename)+", "+str(line_number))
            print(userFilename)
            message == ""
